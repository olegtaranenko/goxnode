...
var MtGoxClient = function(options) {
  ...
  var self = this;
  var apiKeyConfig = options.apiKey.split('-').join(''),
    apiKey = new Buffer(apiKeyConfig, 'hex'),
//    apiKey = buf.toString(),
...
//    var len = apiKey.length + digest.length + queryBuf.length;

    var uncoded = Buffer.concat([apiKey, digest, queryBuf]);

/*
    apiKey.copy(uncoded);
    Log.debug('uncoded', uncoded);
    digest.copy(uncoded, (len = apiKey.length));
    Log.debug('uncoded', uncoded);

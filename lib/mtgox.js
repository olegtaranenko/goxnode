...
//    self.emit('error', new Error(error));
  ...
  self.subscribePrivateChannel = function(optionalIdKey) {
    if (!optionalIdKey) {
      var idKeyPath = '1/generic/private/idkey';
      this.queryHttps(idKeyPath, function(err, result) {
        if (err) {
          Log.error('Error by call to ', idKeyPath, 'error => ', err);
          return;
        }
//        Log.debug('Call ', idKeyPath, 'result => ', JSON.stringify(result));

        doSubscribe(result);
      });
    } else {
      doSubscribe(optionalIdKey);
    }

    function doSubscribe(idKey) {
      var message = {
        "op": "mtgox.subscribe",
        "key": idKey
      };

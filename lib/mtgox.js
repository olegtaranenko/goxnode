...
//    self.emit('data', data);
    ...
    if (data.op == 'subscribe') {
      ...
      if (data.private == 'ticker') {
        processBidders(data.ticker);
      }
      if (data.private != 'ticker' && data.private != 'user_order') {
...
//      Log.debug('body => ', body);
  ...
  self.stopBidding = function(bidderId) {
    var bidder = findBidder(bidderId),
      bidderIndex = autoBidders.indexOf(bidder);

    bidder.stop();
    autoBidders.splice(bidderIndex, 1);
  };
